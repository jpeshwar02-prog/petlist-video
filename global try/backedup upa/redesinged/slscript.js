document.addEventListener("DOMContentLoaded", () => {
  ...
  // ✅ Config object: change per page
  const config = {
    type: "horse",       // animal type
    gender: "stallion",  // gender filter (optional)
  ...
  async function loadItems() {
    try {
      ...
      // ✅ Filter by config
      items = data.filter(a => {
        const typeMatch = !config.type || (a.type && a.type.toLowerCase() === config.type);
        const genderMatch = !config.gender || a.gender === config.gender;
  ...
  function populateFilters() {
    ...
    if (cityFilter) fillSelect(cityFilter, items.map(s => s.city));
    if (bloodlineFilter) fillSelect(bloodlineFilter, items.map(s => s.bloodline));
    if (colorFilter) fillSelect(colorFilter, items.map(s => s.color));
    if (winnerFilter) fillSelect(winnerFilter, items.map(s => s.winner ? "Yes" : "No"));
  }
  ...
  function applyFilters() {
    ...
    return items.filter(s => {
      const win = String(s.winner).toLowerCase();
  ...
  function renderPage(list, page) {
    ...
    // ✅ Use renderCards from shared.js
    renderCards(pagedList, ".cards-container");
