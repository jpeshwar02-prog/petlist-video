...
(function() {
  const slider = document.querySelector('.stallion-slider');
  const cards = slider ? Array.from(slider.querySelectorAll('.stallion-card')) : [];
  const nav = document.querySelector('.stallion-nav');
...
/* ======================== More Pets Slider (Mobile Only) ======================== */
(function() {
  const slider = document.querySelector('.more-pets .stallion-slider');
  const cards = slider ? Array.from(slider.querySelectorAll('.stallion-card')) : [];
  const nav   = document.querySelector('.more-pets .stallion-nav');
  if (!slider || !cards.length || !nav) return;
  ...
  function createNav() {
    ...
    cards.forEach((_, i) => {
      const dash = document.createElement('span');
      dash.textContent = '_'; // underscore nav like horses section
      if (i === 0) dash.classList.add('active');
      dash.addEventListener('click', () => scrollToCard(i));
      nav.appendChild(dash);
    });
